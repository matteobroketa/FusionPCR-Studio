...
describe('protocol calculations', () => {
  ...
  it('builds an equimolar protocol plan', () => {
    const plan = buildProtocolPlan(
      ...
      ['A_outer_F', 'A_inner_R', 'B_inner_F', 'B_outer_R'],
      'q5',
      {
        dntpMillimolar: 0.2,
        dmsoPercent: 0,
    ...
    expect(
      plan.primerUsage.find((entry) => entry.primerName === 'A_outer_F')
        ?.reactionsUsingPrimer,
    ).toBe(3);
    expect(
      plan.reactionMixes.find((entry) => entry.name === 'Fusion PCR')
        ?.cycleCount,
    ).toBe(22);
    expect(
      plan.reactionRecipes
        .find((entry) => entry.name === 'PCR 1A')
        ?.entries.some((entry) => entry.role === 'master-mix'),
    ).toBe(true);
  });
  ...
  it('builds separate-component recipes for phusion plus', () => {
    ...
    const pcr1aRecipe = plan.reactionRecipes.find(
      (entry) => entry.name === 'PCR 1A',
    );
    expect(pcr1aRecipe?.entries.some((entry) => entry.role === 'buffer')).toBe(
      true,
    );
    expect(pcr1aRecipe?.entries.some((entry) => entry.role === 'dntp')).toBe(
      true,
    );
    expect(
      pcr1aRecipe?.entries.some((entry) => entry.role === 'polymerase'),
    ).toBe(true);
    expect(pcr1aRecipe?.entries.some((entry) => entry.role === 'dmso')).toBe(
      true,
    );
    expect(
      pcr1aRecipe?.entries.find(
        (entry) => entry.label === 'Fragment A source template',
      )?.perReactionVolumeUl,
    ).toBeCloseTo(1.5, 6);
  });
  ...
  it('scales fusion template requirements by overfilled reaction count while preserving per-reaction loading', () => {
    ...
    const fusionMix = plan.reactionMixes.find(
      (entry) => entry.name === 'Fusion PCR',
    );
    const fusionRecipe = plan.reactionRecipes.find(
      (entry) => entry.name === 'Fusion PCR',
    );
    const stageAEntry = plan.stageMixEntries.find(
      (entry) => entry.label === 'PCR 1A product',
    );
    const stageATemplate = fusionRecipe?.entries.find(
      (entry) => entry.label === 'PCR 1A product template',
    );

    expect(fusionMix?.overfilledReactionCount).toBeCloseTo(3.6, 6);
    expect(stageAEntry?.targetPmol).toBeCloseTo(0.144, 6);
    expect(stageATemplate?.totalVolumeUl).toBeCloseTo(
      stageAEntry?.requiredVolumeUl ?? 0,
      6,
    );
