...
export function buildProtocolText(design: FusionDesign): string {
  ...
  return [
    ...
    `Approximate design quality score: ${design.qualityScore.toFixed(3)} (heuristic; not experimentally calibrated)`,
    ...
    '',
    'Approximate secondary structure',
    ...design.primers.map(
      (primer) =>
...
export function buildValidationReport(design: FusionDesign): string {
  ...
  return [
    ...
    '',
    `Design mode: ${design.project.mode}`,
    'Approximation note: structure and quality outputs are heuristic calculations, not experimentally calibrated probabilities of success.',
    '',
