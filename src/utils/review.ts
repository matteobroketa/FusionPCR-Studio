...
export type WorkflowStage =
  'overview' | 'pcr1a' | 'pcr1b' | 'fusion' | 'verification';
...
export function summarizeDesignComparison(
  design: FusionDesign,
...
export function getStagePrimerNames(
  design: FusionDesign,
  stage: WorkflowStage,
): string[] {
  ...
  const reactionName = getWorkflowStageLabel(stage) as
    'PCR 1A' | 'PCR 1B' | 'Fusion PCR';
  return (
    design.reactions.find((reaction) => reaction.name === reactionName)
      ?.primerNames ?? []
  );
}

export function getStageSequencePreviews(
  design: FusionDesign,
  stage: WorkflowStage,
  ...
  switch (stage) {
    case 'pcr1a':
      return [
        {
          label: 'Fragment A selected slice',
          sequence: design.effectiveSelectedA,
        },
    ...
    case 'pcr1b':
      return [
        {
          label: 'Fragment B selected slice',
          sequence: design.effectiveSelectedB,
        },
    ...
    case 'verification':
      return [
        ...
        {
          label: 'Simulated final fusion product',
          sequence: design.finalProduct,
        },
      ];
    default:
      return [
...
export function buildJunctionSummary(
  design: FusionDesign,
  flankLength = 14,
): JunctionSummary {
