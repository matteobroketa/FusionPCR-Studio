...
export function trimFragment(
  ...
  const nextSequence =
    side === 'left'
      ? sequence.slice(trimmedAmount)
      : sequence.slice(0, sequence.length - trimmedAmount);
  const nextStart = Math.max(
    1,
    Math.min(
      fragment.start - (side === 'left' ? trimmedAmount : 0),
  ...
  const nextEnd = Math.max(
    nextStart,
    Math.min(
      fragment.end - (side === 'left' ? trimmedAmount : 0),
      nextSequence.length || 1,
    ),
  );
...
export function deleteSelectedRange(fragment: FragmentInput): FragmentInput {
  ...
  const nextEnd = nextSequence.length
    ? Math.min(start, nextSequence.length)
    : 1;
  return withSequenceMetadata(
    fragment,
    nextSequence,
    Math.min(start, nextEnd),
    nextEnd,
  );
}
...
export function replaceSelectedRange(
  ...
  const nextStart = payload.length
    ? start
    : Math.min(start, nextSequence.length || 1);
  const nextEnd = payload.length
    ? start + payload.length - 1
    : Math.min(nextStart, nextSequence.length || 1);
  return withSequenceMetadata(fragment, nextSequence, nextStart, nextEnd);
}
...
export function insertAtPosition(
  ...
  const safePosition = Math.max(
    1,
    Math.min(Math.floor(position), sequence.length + 1),
  );
  ...
  const nextStart = payload.length
    ? safePosition
    : Math.min(safePosition, nextSequence.length || 1);
  const nextEnd = payload.length
    ? safePosition + payload.length - 1
    : Math.min(nextStart, nextSequence.length || 1);
  return withSequenceMetadata(fragment, nextSequence, nextStart, nextEnd);
}
...
export function splitFragment(
  ...
  const sequence = normalizeSequence(fragment.sequence);
  const safeCoordinate = Math.max(
    1,
    Math.min(Math.floor(coordinate), Math.max(sequence.length - 1, 1)),
  );
  const leftSequence = sequence.slice(0, safeCoordinate);
  const rightSequence = sequence.slice(safeCoordinate);
  const left = withSequenceMetadata(
    { ...fragment, label: leftLabel ?? `${fragment.label} left` },
    leftSequence,
    1,
    leftSequence.length || 1,
  );
  const rightBase = createEmptyFragment(
    rightLabel ?? `${fragment.label} right`,
  );
  const right = withSequenceMetadata(
    { ...rightBase, label: rightLabel ?? `${fragment.label} right` },
    rightSequence,
    1,
    rightSequence.length || 1,
  );
  return [left, right];
}
