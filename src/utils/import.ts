...
import { checksumSequence, type SequenceFeature, type SequenceTopology, type SourceFormat } from './fusion';
...
function validateDnaSequence(sequenceInput: string): { sequence: string; ambiguousBases: string[] } {
  ...
  if (invalidBases.length) {
    if (detectProteinLikeInput(sequence)) {
      throw new Error('The imported content looks like a protein sequence rather than DNA.');
    }

    const unsupportedAmbiguity = invalidBases.filter((base) => /^[A-Z]$/.test(base));
    if (unsupportedAmbiguity.length) {
      throw new Error(`Unsupported ambiguity symbols: ${unsupportedAmbiguity.join(', ')}. Only A, C, G, T, and N are supported right now.`);
    }

    throw new Error(`Invalid characters were found in the imported sequence: ${invalidBases.join(', ')}`);
  }
...
function parsePlainSequence(rawInput: string): ImportParseResult {
  return {
    format: 'plain',
    records: [buildImportedSource('Imported sequence', rawInput, 'plain', 'linear')],
    warnings: [],
  };
}
...
function parseFasta(rawInput: string): ImportParseResult {
  ...
  const flush = () => {
    ...
    records.push(buildImportedSource(currentName || `Sequence ${records.length + 1}`, currentSequenceLines.join(''), 'fasta', 'linear'));
  };
  ...
  if (duplicateNames.length) {
    throw new Error(`Duplicate sequence names were found in the FASTA import: ${Array.from(new Set(duplicateNames)).join(', ')}`);
  }

  if (records.length > 2) {
    warnings.push('More than two FASTA records were imported; apply the desired records to fragments A and B manually.');
  }
...
function parseGenBankFeatures(featureText: string, topology: SequenceTopology): SequenceFeature[] {
  ...
  for (const line of lines) {
    ...
    if (featureMatch) {
      ...
      const ranges = Array.from(location.matchAll(/(\d+)\.\.(\d+)/g)).map((match) => ({
        start: Number(match[1]),
        end: Number(match[2]),
      }));
      const crossesOrigin =
        topology === 'circular' &&
        ranges.some((range, index) => index > 0 && range.start < ranges[index - 1].start);
    ...
    if (qualifierMatch && currentFeature) {
      ...
      if (qualifierKey === 'label' || qualifierKey === 'gene' || qualifierKey === 'product') {
...
function parseGenBank(rawInput: string): ImportParseResult {
  ...
  const topology = locusMatch?.[2]?.toLowerCase() === 'circular' ? 'circular' : 'linear';
  ...
  const featureSectionMatch = rawInput.match(/^FEATURES\s+Location\/Qualifiers([\s\S]+?)^ORIGIN/im);
  const features = parseGenBankFeatures(featureSectionMatch?.[1] ?? '', topology);
  const sequence = originMatch[1].replace(/[^A-Za-z]/g, '');
  const warnings: string[] = [];

  if (topology === 'circular') {
    warnings.push('Circular topology was detected. Circular source selections can wrap around the origin, while more complex circular editing remains limited.');
  }
  if (features.some((feature) => feature.crossesOrigin)) {
    warnings.push('One or more GenBank features cross the circular origin and are preserved as raw feature locations.');
  }

  return {
    format: 'genbank',
    records: [buildImportedSource(name, sequence, 'genbank', topology, features)],
    warnings,
  };
}
