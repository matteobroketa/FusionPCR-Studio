...
function runRustParity(requests: ParityRequest[]): ParityResponse[] {
  const result = spawnSync(
    'cargo',
    ['run', '--quiet', '-p', 'fusion-core', '--bin', 'parity-cli'],
    {
      cwd: process.cwd(),
  ...
  return (JSON.parse(result.stdout) as { responses: ParityResponse[] })
    .responses;
}
...
const baseProject = {
  ...
  revision: 1,
  projectHash: 'parity-test-project',
...
describe('TypeScript and Rust engine parity', () => {
  it(
    'matches sequence parsing, reverse-complement, and GC-fraction outputs',
    () => {
      ...
      expect(
        (parsedInvalid as Extract<ParityResponse, { kind: 'parse-sequence' }>)
          .error,
      ).toContain('unsupported bases');
      expect(parsedEmpty).toMatchObject({
        kind: 'parse-sequence',
        ok: false,
      });
      expect(
        (parsedEmpty as Extract<ParityResponse, { kind: 'parse-sequence' }>)
          .error,
      ).toContain('sequence is empty');
      expect(reverseComplementResponse).toEqual({
        kind: 'reverse-complement',
        sequence: reverseComplement('ATGCNN'),
      });
      expect(
        (gcFirst as Extract<ParityResponse, { kind: 'gc-fraction' }>)
          .gc_fraction,
      ).toBeCloseTo(calculateGcPercentage('ATGC') / 100, 3);
      expect(
        (gcSecond as Extract<ParityResponse, { kind: 'gc-fraction' }>)
          .gc_fraction,
      ).toBeCloseTo(calculateGcPercentage('GGCCAA') / 100, 3);
    },
    PARITY_TIMEOUT_MS,
  );

  it(
    'matches exact-fusion and approved protein-fusion target construction',
    () => {
      const exactProject = {
        mode: 'exact' as const,
      ...
      const exactTs = buildFusionDesign({
        ...
        changeApprovals: {
          ...defaultChangeApprovals(),
          removeUpstreamStop:
            exactProject.change_approvals.remove_upstream_stop,
          removeDownstreamStart:
            exactProject.change_approvals.remove_downstream_start,
        },
        fragmentA: makeFragment(
          'A',
          exactProject.fragment_a.sequence,
          exactProject.fragment_a.start,
          exactProject.fragment_a.end,
        ),
        fragmentB: makeFragment(
          'B',
          exactProject.fragment_b.sequence,
          exactProject.fragment_b.start,
          exactProject.fragment_b.end,
        ),
      });
      ...
      const proteinTs = buildFusionDesign({
        ...
        changeApprovals: {
          ...defaultChangeApprovals(),
          removeUpstreamStop:
            proteinProject.change_approvals.remove_upstream_stop,
          removeDownstreamStart:
            proteinProject.change_approvals.remove_downstream_start,
        },
        fragmentA: makeFragment(
          'A',
          proteinProject.fragment_a.sequence,
          proteinProject.fragment_a.start,
          proteinProject.fragment_a.end,
        ),
        fragmentB: makeFragment(
          'B',
          proteinProject.fragment_b.sequence,
          proteinProject.fragment_b.start,
          proteinProject.fragment_b.end,
        ),
      });
  ...
  it(
    'matches protocol unit conversions',
    () => {
      ...
      expect(
        (response as Extract<ParityResponse, { kind: 'protocol-conversions' }>)
          .mass_ng,
      ).toBeCloseTo(pmolToMassNg(0.05, 500), 10);
      expect(
        (response as Extract<ParityResponse, { kind: 'protocol-conversions' }>)
          .pmol,
      ).toBeCloseTo(massNgToPmol(33.0, 500), 10);
      expect(
        (response as Extract<ParityResponse, { kind: 'protocol-conversions' }>)
          .volume_ul,
      ).toBeCloseTo(volumeForMass(33.0, 11.0), 10);
    },
    PARITY_TIMEOUT_MS,
  );
});
